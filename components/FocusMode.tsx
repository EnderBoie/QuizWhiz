...
export const FocusMode: React.FC<FocusModeProps> = ({ user, quizzes, onBack, onStartQuiz }) => {
  ...
  const handleGenerateFocus = async () => {
    ...
    if (mistakes.length === 0) {
        setError("You haven't made enough mistakes yet to generate a focus session! Go play some quizzes.");
        return;
    }
    ...
    try {
        const result = await generateFocusSession(mistakes);
        ...
        console.error(err);
        setError("Failed to generate focus session. Please try again.");
    } finally {
  ...
  const startFocusQuiz = () => {
      ...
      const focusQuiz: Quiz = {
          id: Date.now(),
          userId: user.id,
          title: "⚡ Focus Session: Personalized Study",
          questions: analysisResult.questions,
          createdAt: new Date().toISOString(),
          theme: 'nature', // calming theme for study
        ...
        {!analysisResult ? (
            <div className="flex-1 flex flex-col items-center justify-center text-center animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="relative mb-8 group">
                    ...
                    {wrongAnswerCount > 0 && (
                        <div className="absolute -top-2 -right-2 bg-red-500 text-white text-xs font-bold px-2 py-1 rounded-full border-2 border-slate-900">
                            {wrongAnswerCount} Issues Found
                        </div>
                    )}
                </div>

                <h2 className="text-3xl font-black mb-4">Target Your Weaknesses</h2>
                <p className="text-slate-400 text-lg max-w-lg mb-8 leading-relaxed">
                    AI will analyze your recent quiz history, identify gaps in your knowledge, 
                    and create a custom 5-question drilling session just for you.
                </p>
                ...
                <button
                    ...
                    disabled={isAnalyzing || wrongAnswerCount === 0}
                ...
                {wrongAnswerCount === 0 && (
                    <p className="mt-4 text-slate-500 text-sm">
                        (Play some quizzes first to generate data!)
                    </p>
                )}
            </div>
        ) : (
                ...
                <div className="bg-white/5 border border-white/10 rounded-3xl p-8 flex flex-col sm:flex-row items-center justify-between gap-6">
                    <div>
                        ...
                        <p className="text-slate-400">
                            We've generated <span className="text-white font-bold">5 questions</span> targeting these topics.
                        </p>
                    </div>
                    <button 
